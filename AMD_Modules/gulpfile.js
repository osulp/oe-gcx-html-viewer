...
async function bundleFiles() {
    if (tsCompileFailed) {
        console.warn(color.yellow("Skipping module bundling due to errors in TypeScript compile"));
        return;
    }
...
${bundledShims}
    ...
    console.log(color.green(`Build completed successfully in ${buildTimeSeconds}s`));
}
...
function processsDeclarationFiles(foundReferences, namespace) {
    return through.obj((file, encoding, callback) => {
        ...
        const relativeFilePath = file.path.replace(base, "").replace(/\\/g, "/");;
...
gulp.task("ts-build", () => {
    ...
    const tsResult = gulp.src(glob)
        ...
        .on("finish", () => {
            ...
            if (tsCompileFailed) {
                console.error(color.red(`TypeScript compile failed with ${errorCount} errors`));
...
gulp.task("bundle-modules", gulp.series("ts-build", bundleFiles));
...
gulp.task("watch", gulp.series("bundle-modules", () => {
...
gulp.task('default', gulp.series("bundle-modules"));
